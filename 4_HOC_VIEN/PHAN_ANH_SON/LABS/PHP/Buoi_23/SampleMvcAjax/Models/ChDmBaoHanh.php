...
class ChDmBaoHanh{
	...
	public function FindKey($Key){
		$params = array($Key);
		return $this->DB->callProcedure('p_ch_dm_baohanh_find_key(?)',$params);
	}
	
	public function GetById(){
		$params = array($this->Id);
		return $this->DB->callProcedure('p_ch_dm_baohanh_get_byid(?)',$params);
	}
	
	public function Save(){
		$params = array($this->Id, $this->Ten,$this->MoTa, $this->TrangThai);
		return $this->DB->callProcedure('p_ch_dm_baohanh_save(?,?,?,?)',$params);
	}
	
	public function Del(){
		$params = array($this->Id);
		return $this->DB->callProcedure('p_ch_dm_baohanh_del(?)',$params);
	}
