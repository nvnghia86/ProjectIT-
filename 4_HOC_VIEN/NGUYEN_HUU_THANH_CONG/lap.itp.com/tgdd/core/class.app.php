...
//        exit();
        $this->TAI_KHOAN = !empty($_SESSION["auth"]["TAI_KHOAN"]) ? $_SESSION["auth"]["TAI_KHOAN"] : "";
        if ($this->TAI_KHOAN == "") {
    ...
    public function display() {
        ...
        if (empty($this->layout)) {
            if (!empty($this->view))
                require(DIR_MODULE . DS . "views" . DS . $this->view . '.php');
        }else {
            ob_start();
            if (!empty($this->view)){
                $_dir = DIR_MODULE . DS . "views" . DS . $this->view . '.php';
                if(file_exists($_dir)){
                    require($_dir);
                }else{
                    die('Yêu cầu (view) không hợp lệ.');
                } 
                
                $_dirApp = DIR_MODULE . DS . 'app.php';
                if(file_exists($_dirApp)){
                    require_once($_dirApp);
                }
            }
